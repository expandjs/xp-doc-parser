 ...
 */
module.exports = new XP.Class('XPDocParser', {
    ...
    initialize: {
        ...
        value(options, resolver) {
            ...
            // Listening
            this.request.once('data', this._handleData.bind(this));
            this.request.once('error', error => this.error = error);
            this.request.on('state', state => this.state = state);

            // Resolving
            this.once('data', data => resolver(null, data));
            this.once('error', error => resolver(error, null));
     ...
     */
    inherited: {
        set(val) { return XP.isDefined(this.inherited) ? this.inherited : Boolean(val); }
    },
     ...
     */
    request: {
        set(val) { return this.request || val; },
    ...
    _handleData(data) {
        ...
        if (parsed && parsed.extends && parsed.extends.url) {
            ...
            parser.once('data', () => (count += 1) === parsers.length && (this.data = this._sortData(this._mergeData(parsed, parsers), true)));
            parsers.push(parser);
        }
...
// Globalizing
if (typeof window !== "undefined") { window.XPDocParser = module.exports; }
